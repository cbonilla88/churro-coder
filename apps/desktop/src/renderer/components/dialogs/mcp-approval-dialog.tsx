...
export function McpApprovalDialog() {
  ...
  const handleAllowAll = async () => {
    ...
    const samePlugin = pendingApprovals.filter((a) => a.pluginSource === currentApproval.pluginSource);
        ...
        <AlertDialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <AlertDialogDescription>A plugin wants to connect to an MCP server</AlertDialogDescription>
            </div>
          </div>
        </AlertDialogHeader>
          ...
          <div className="space-y-3">
            <div className="rounded-md border border-border bg-muted/30 p-3 space-y-2">
              <div className="flex gap-2">
                ...
                <span className="text-xs text-foreground font-medium">{currentApproval.pluginSource}</span>
              </div>
              <div className="flex gap-2">
                <span className="text-xs text-muted-foreground w-14 shrink-0">Server</span>
                <span className="text-xs text-foreground font-mono">{currentApproval.serverName}</span>
              </div>
              {command && (
                <div className="flex gap-2">
