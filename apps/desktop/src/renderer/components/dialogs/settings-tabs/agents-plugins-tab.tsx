          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
                ...
                <Switch checked={!plugin.isDisabled} onCheckedChange={onToggleEnabled} disabled={isTogglingEnabled} />
              </div>
            </div>
            {plugin.category && <p className="text-xs text-muted-foreground mt-0.5 capitalize">{plugin.category}</p>}
          </div>
...
function PluginListItem({
  ...
  return (
    <button
      ...
      {plugin.description && (
        <div className="text-[11px] text-muted-foreground/60 truncate mt-0.5">{plugin.description}</div>
      )}
    </button>
  );
}
...
export function AgentsPluginsTab() {
  ...
  const filteredPlugins = useMemo(() => {
    ...
    return plugins.filter((p) => {
      ...
      if (
        p.components.commands.some((c) => c.name.toLowerCase().includes(q) || c.description?.toLowerCase().includes(q))
      )
        return true;
      if (p.components.skills.some((c) => c.name.toLowerCase().includes(q) || c.description?.toLowerCase().includes(q)))
        return true;
      if (p.components.agents.some((c) => c.name.toLowerCase().includes(q) || c.description?.toLowerCase().includes(q)))
        return true;
          ...
          <div
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-center px-4">
                ...
                <p className="text-[11px] text-muted-foreground/70">Install plugins to ~/.claude/plugins/</p>
              </div>
