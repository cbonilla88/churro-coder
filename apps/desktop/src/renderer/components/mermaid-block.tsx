...
const MermaidBlockInner = memo(function MermaidBlockInner({ code }: { code: string }) {
  ...
  const renderDiagram = useCallback(() => {
    ...
    const looksIncomplete =
      hasUnclosedBrackets(code) || hasUnclosedBraces(code) || hasUnclosedParens(code) || hasUnclosedQuotes(code);
    ...
    <>
      <div className="relative mt-2 mb-4 rounded-[10px] bg-muted/50 overflow-hidden">
        ...
        <div className="absolute top-[6px] right-[6px] flex gap-1 z-[2]">
          <button onClick={handleCopy} tabIndex={-1} className="p-1" title={copied ? 'Copied!' : 'Copy code'}>
          ...
          {renderState.status === 'success' && (
            <>
              ...
              <button onClick={openFullscreen} tabIndex={-1} className="p-1" title="View fullscreen">
        ...
        {/* Content */}
        <div className="p-4 min-h-[60px] flex items-center justify-center">
          {renderState.status === 'idle' && <div className="text-muted-foreground text-sm">Waiting for diagram...</div>}
