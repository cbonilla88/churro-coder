...
export function useRenameRemoteSubChat() {
  ...
  return useMutation({
    mutationFn: ({ subChatId, name }: { subChatId: string; name: string }) => remoteApi.renameSubChat(subChatId, name),
    onSuccess: () => {
...
export function useRenameRemoteChat() {
  ...
  return useMutation({
    mutationFn: ({ chatId, name }: { chatId: string; name: string }) => remoteApi.renameChat(chatId, name),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['remote-chats', teamId] });
      queryClient.invalidateQueries({ queryKey: ['remote-chat'] });
    }
