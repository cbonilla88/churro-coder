          ...
          <Tooltip delayDuration={500}>
            <TooltipTrigger asChild>
              <div className="flex-1 min-w-0 flex items-center overflow-hidden">
                {dirPath && (
                  <span className="text-xs text-muted-foreground truncate flex-shrink min-w-0">{dirPath}/</span>
                )}
      ...
      <ContextMenuContent className="w-64">
        {showMultiDiscard ? (
          <>
            ...
            <ContextMenuItem onClick={onCopySelectedRelativePaths}>Copy Relative Paths</ContextMenuItem>
          </>
        ) : (
          <>
  ...
  const filteredFiles = useMemo(() => {
    ...
    if (subChatFilterPaths) {
      result = result.filter(({ file }) =>
        subChatFilterPaths.some((filterPath) => matchesFilePath(file.path, filterPath))
      );
    }
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                            ...
                            This chat
                            <span className="ml-1.5 text-[10px] text-muted-foreground">{scopedCount}</span>
                          </Button>
                        </TooltipTrigger>
