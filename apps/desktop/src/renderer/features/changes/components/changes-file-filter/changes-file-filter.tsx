                ...
                <TooltipTrigger asChild>
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      <CircleFilterIcon
                        className={cn('h-4 w-4', selectedSubChatId ? 'text-primary' : 'text-muted-foreground')}
                      />
                    </Button>
                  </PopoverTrigger>
                </TooltipTrigger>
      ...
      {selectedSubChat && (
        <div className="flex items-center justify-between gap-2 h-7 px-2 rounded-md bg-muted/80 border border-border/50">
          <div className="flex items-center gap-1.5 min-w-0">
            <span className="text-[10px] text-muted-foreground flex-shrink-0 uppercase tracking-wide">Filtered</span>
            <span className="text-muted-foreground/30 flex-shrink-0">•</span>
            <span className="text-xs text-foreground/80 truncate">{selectedSubChat.name || 'New Chat'}</span>
            <span className="text-[10px] text-muted-foreground flex-shrink-0">({selectedSubChat.fileCount})</span>
          </div>
          <button
            type="button"
