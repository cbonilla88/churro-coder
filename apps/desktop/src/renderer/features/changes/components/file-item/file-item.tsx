  ...
  const fileContent = (
    <div
      ...
      <button
        ...
        <span className={cn('shrink-0 flex items-center text-xs', statusBadgeColor)}>{statusIndicator}</span>
        <span className="flex-1 min-w-0 flex items-center gap-1">
          <Tooltip>
            <TooltipTrigger asChild>
              <span className="text-xs text-start truncate overflow-hidden text-ellipsis">{fileName}</span>
            </TooltipTrigger>
          ...
          {showStatsDisplay && (
            <span className="flex items-center gap-0.5 text-[10px] font-mono shrink-0 whitespace-nowrap opacity-60">
              {file.additions > 0 && <span className="text-green-600 dark:text-green-500">+{file.additions}</span>}
    ...
    <>
      ...
      <AlertDialog open={showDiscardDialog} onOpenChange={setShowDiscardDialog}>
        <AlertDialogContent className="w-[340px]">
          ...
          <AlertDialogDescription className="px-5 pb-5">{discardDialogDescription}</AlertDialogDescription>
          <AlertDialogFooter>
