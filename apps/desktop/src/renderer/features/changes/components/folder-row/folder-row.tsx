  ...
  return (
    <>
      ...
      <div className="flex items-center gap-1 flex-1 min-w-0">
        <span
          className={cn('truncate', isGrouped ? 'w-0 grow text-left' : 'flex-1 min-w-0 text-xs text-foreground')}
          dir={isGrouped ? 'rtl' : undefined}>
          {name}
        </span>
        {fileCount !== undefined && (
          <span className="text-[10px] text-muted-foreground shrink-0 tabular-nums">{fileCount}</span>
        )}
      </div>
    </>
  );
...
export function FolderRow({
  ...
  return (
    <CollapsibleRow
      ...
      header={<FolderRowHeader name={name} level={level} fileCount={fileCount} isGrouped={isGrouped} />}>
      {children}
    </CollapsibleRow>
  );
}
