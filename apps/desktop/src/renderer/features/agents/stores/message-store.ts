...
export const getPerChatMessageKey = (subChatId: string, messageId: string) => `${subChatId}:::${messageId}`;
...
export const userMessageIdsAtom = atom((get) => {
  ...
  if (cached && newUserIds.length === cached.length && newUserIds.every((id, i) => id === cached[i])) {
    return cached;
  }
...
export const messageTokenDataAtom = atom((get) => {
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    if (
      parts?.some(
        (part) => part.type === 'tool-Compact' && (part.state === 'output-available' || part.state === 'result')
      )
    ) {
...
export const syncMessagesWithStatusAtom = atom(
  ...
  (get, set, payload: { messages: Message[]; status: string; subChatId?: string; updateGlobal?: boolean }) => {
    ...
    if (updateGlobal) {
      ...
      globalIdsChanged = newIds.length !== currentIds.length || newIds.some((id, i) => id !== currentIds[i]);
    ...
    const perChatIdsChanged = newIds.length !== perChatIds.length || newIds.some((id, i) => id !== perChatIds[i]);
