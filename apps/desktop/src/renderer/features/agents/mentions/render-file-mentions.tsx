...
export function FileOpenProvider({ onOpenFile, children }: { onOpenFile: FileOpenHandler; children: React.ReactNode }) {
...
function parseMention(id: string): ParsedMention | null {
  ...
  if (!isFile && !isFolder && !isSkill && !isAgent && !isTool && !isQuote && !isDiff && !isPasted && !isChatHistory)
    return null;
  ...
  return useMemo(() => {
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      if (match.index > lastIndex) {
        nodes.push(<span key={`text-${key++}`}>{renderTextWithUltrathink(text.slice(lastIndex, match.index))}</span>);
      }
    ...
    if (lastIndex < text.length) {
      nodes.push(<span key={`text-end-${key}`}>{renderTextWithUltrathink(text.slice(lastIndex))}</span>);
    }
...
export function TextMentionBlock({ mention }: { mention: ParsedMention }) {
  ...
  return (
    <div className="flex items-center gap-2 pl-1 pr-2 py-1 rounded-lg bg-muted/50 cursor-default min-w-[120px] max-w-[200px]">
      <div className="flex items-center justify-center w-8 self-stretch rounded-md bg-muted shrink-0">{icon}</div>
...
export function TextMentionBlocks({ mentions }: { mentions: ParsedMention[] }) {
  const textMentions = mentions.filter(
    (m) => m.type === 'quote' || m.type === 'diff' || m.type === 'pasted' || m.type === 'chatHistory'
  );
