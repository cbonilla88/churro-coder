  ...
  return (
    <div
      ...
      <div className="w-[128px] flex items-center justify-end shrink-0">
        {countInfo.total > 0 ? (
          <>
            <span className="text-xs text-muted-foreground mr-1">{`${countInfo.current} of ${countInfo.total}`}</span>
            <button
              type="button"
          ...
          inputValue.trim() && searchCompleted && <span className="text-xs text-muted-foreground">No results</span>
        )}
      </div>
