      ...
      <CollapsibleContent>
        <div className="mt-1 mx-2 rounded-md border bg-muted/30 overflow-hidden">
          ...
          <div className="p-3 max-h-[300px] overflow-auto">
            {highlightedHtml ? (
              <pre className="text-xs font-mono" dangerouslySetInnerHTML={{ __html: highlightedHtml }} />
            ) : (
              <pre className="text-xs font-mono text-muted-foreground whitespace-pre-wrap">{jsonString}</pre>
            )}
          </div>
        </div>
      </CollapsibleContent>
