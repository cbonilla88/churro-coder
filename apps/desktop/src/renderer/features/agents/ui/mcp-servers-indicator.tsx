...
export const McpServersIndicator = memo(function McpServersIndicator({ projectPath }: McpServersIndicatorProps) {
  ...
  const getStatusIcon = (status: MCPServerStatus) => {
    switch (status) {
      ...
      case 'needs-auth':
        return <span className="w-2 h-2 rounded-full bg-yellow-500" aria-label="Needs authentication" />;
      case 'pending':
        return <Loader2 className="w-3 h-3 text-muted-foreground animate-spin" aria-label="Connecting" />;
      default:
        return <span className="w-2 h-2 rounded-full bg-muted-foreground/50" aria-label="Unknown status" />;
    }
  };
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent
        ...
        <div className="max-h-64 overflow-y-auto py-1" role="list" aria-labelledby="mcp-servers-title">
          ...
          <>
            ...
            <div className="pb-1" role="list" aria-labelledby="plugins-title">
              {sessionInfo.plugins.map((plugin) => (
                <div key={plugin.path} className="px-3 py-1.5 text-sm flex items-center gap-2" role="listitem">
