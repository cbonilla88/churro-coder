...
export function SubChatContextMenu({
  ...
  return (
    <ContextMenuContent className="w-48">
      ...
      {chatId && (
        <ContextMenuSub>
          ...
          <ContextMenuSubContent sideOffset={6} alignOffset={-4}>
            <ContextMenuItem onClick={() => handleExport('markdown')}>Download as Markdown</ContextMenuItem>
            ...
            <ContextMenuSeparator />
            <ContextMenuItem onClick={() => handleCopy('markdown')}>Copy as Markdown</ContextMenuItem>
      ...
      {isSplitTab ? (
        <>
          {splitPaneCount > 2 && onRemoveFromSplit && (
            <ContextMenuItem onClick={() => onRemoveFromSplit(subChat.id)}>Remove from split</ContextMenuItem>
          )}
      ...
      {showCloseTabOptions ? (
        <>
          <ContextMenuItem onClick={() => onCloseTab?.(subChat.id)} className="justify-between" disabled={isOnlyChat}>
            Close chat
        ...
        <>
          <ContextMenuItem onClick={() => onArchive(subChat.id)} className="justify-between" disabled={isOnlyChat}>
            Archive chat
            {!isOnlyChat && <Kbd>{closeTabShortcut}</Kbd>}
          </ContextMenuItem>
          <ContextMenuItem
