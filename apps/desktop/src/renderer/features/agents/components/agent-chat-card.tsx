...
function GitHubAvatar({ gitOwner, className = 'h-4 w-4' }: { gitOwner: string; className?: string }) {
    ...
    <div className="relative flex-shrink-0 h-4 w-4">
      ...
      <div
        ...
        {isLoading ? (
          ...
          <PlanIcon className={cn('w-2.5 h-2.5', isSelected ? 'text-primary-foreground' : 'text-muted-foreground')} />
        ) : (
          <AgentIcon className={cn('w-2.5 h-2.5', isSelected ? 'text-primary-foreground' : 'text-muted-foreground')} />
        )}
      </div>
    </div>
  );
...
export function AgentChatCard({
  ...
  const lastMode = lastChatModes.get(chat.id) || 'execute';
    ...
    <div
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          <span className="truncate block text-sm leading-tight">{chat.name || 'Untitled Chat'}</span>
        </div>
      </div>
    </div>
  );
