...
export function TerminalSearch({ searchAddon, isOpen, onClose }: TerminalSearchProps) {
  ...
  return (
    <div className="absolute top-2 right-2 z-10 flex items-center gap-1 rounded-md border border-border bg-background p-1.5 shadow-lg">
      ...
      />
      {matchCount !== null && <span className="px-1 text-xs text-muted-foreground">{matchCount} matches</span>}
      <button
      ...
      <button onClick={() => handleSearch('next')} className="rounded p-1 hover:bg-muted" title="Next match (Enter)">
