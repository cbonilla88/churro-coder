  ...
  return (
    <ContextMenu>
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-0.5">
              <div className="flex items-center gap-1">
                <span className="truncate block text-sm leading-tight flex-1">{chat.name || 'Untitled'}</span>
      ...
      <ContextMenuContent className="w-48">
        ...
        <ContextMenuSeparator />
        <ContextMenuItem onClick={(e) => onArchive(e as unknown as React.MouseEvent)}>Archive</ContextMenuItem>
...
export function InboxView() {
  ...
  const markReadMutation = useMutation({
    mutationFn: (executionId: string) => remoteTrpc.automations.markInboxItemRead.mutate({ executionId }),
    onSuccess: () => {
  ...
  const filteredChats = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      chats = chats.filter(
        (chat) => chat.name.toLowerCase().includes(query) || chat.automationName.toLowerCase().includes(query)
      );
    }
      ...
      <div className="flex h-full overflow-hidden" data-inbox-page>
        ...
        <ResizableSidebar
          ...
          <div
            ...
            {sidebarOpen && <TrafficLightSpacer isFullscreen={isFullscreen} isDesktop={isDesktop} />}
