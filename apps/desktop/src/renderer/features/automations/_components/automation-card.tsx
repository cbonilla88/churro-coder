    ...
    <div
      ...
      <div className="flex items-center gap-1.5 mb-3">
        {platforms.map((platform, idx) => (
          <div key={idx} className="w-7 h-7 rounded-md bg-accent/50 flex items-center justify-center">
      ...
      <div className="flex flex-col gap-0">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-foreground line-clamp-2">{automation.name}</span>
          {!automation.is_enabled && (
            <span className="text-[10px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground">Paused</span>
          )}
        </div>
        <p className="text-xs text-muted-foreground line-clamp-2 mt-0.5">{getAutomationDescription(triggers)}</p>
      </div>
    </div>
  );
}
