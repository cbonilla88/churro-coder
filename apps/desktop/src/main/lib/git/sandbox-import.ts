  ...
  try {
    ...
    if (exportData.bundle) {
      ...
      try {
        ...
        if (isFullExport) {
          ...
          await execFileAsync('git', ['-C', worktreePath, 'fetch', bundlePath, 'HEAD:sandbox-import-temp'], {
            timeout: 60_000
          });
    ...
    if (exportData.stagedPatch) {
      ...
      try {
        ...
        // Apply and stage
        await execFileAsync('git', ['-C', worktreePath, 'apply', '--cached', stagedPatchPath], { timeout: 60_000 });
