...
export async function applyRollbackStash(worktreePath: string, sdkMessageUuid: string): Promise<RollbackResult> {
  try {
    ...
    if (!indexTree || !worktreeTree) {
      console.error(`[claude] Rollback checkpoint missing tree metadata for sdkMessageUuid=${sdkMessageUuid}`);
