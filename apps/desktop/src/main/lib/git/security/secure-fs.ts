...
import { assertRegisteredWorktree, PathValidationError, resolvePathInWorktree } from './path-validation';
...
function isPathWithinWorktree(worktreeReal: string, targetReal: string): boolean {
  ...
  const escapesWorktree = relativePath === '..' || relativePath.startsWith(`..${sep}`) || isAbsolute(relativePath);
...
async function assertParentInWorktree(worktreePath: string, fullPath: string): Promise<void> {
  ...
  while (currentPath !== dirname(currentPath)) {
    ...
    try {
      ...
      if (stats.isSymbolicLink()) {
        ...
        const resolvedTarget = isAbsolute(linkTarget) ? linkTarget : resolve(dirname(currentPath), linkTarget);
        ...
        try {
          ...
          if (error instanceof Error && 'code' in error && error.code === 'ENOENT') {
            ...
            if (targetRelative === '..' || targetRelative.startsWith(`..${sep}`) || isAbsolute(targetRelative)) {
              throw new PathValidationError('Dangling symlink points outside the worktree', 'SYMLINK_ESCAPE');
            }
      ...
      if (!isPathWithinWorktree(worktreeReal, parentReal)) {
        throw new PathValidationError('Parent directory resolves outside the worktree', 'SYMLINK_ESCAPE');
      }
  ...
  throw new PathValidationError('Could not validate path ancestry within worktree', 'SYMLINK_ESCAPE');
}
...
async function assertRealpathInWorktree(worktreePath: string, fullPath: string): Promise<void> {
  try {
    ...
    if (!isPathWithinWorktree(worktreeReal, real)) {
      throw new PathValidationError('File is a symlink pointing outside the worktree', 'SYMLINK_ESCAPE');
    }
  } catch (error) {
...
async function assertDanglingSymlinkSafe(worktreePath: string, fullPath: string): Promise<void> {
  ...
  try {
    ...
    if (stats.isSymbolicLink()) {
      ...
      const resolvedTarget = isAbsolute(linkTarget) ? linkTarget : resolve(dirname(fullPath), linkTarget);
...
export const secureFs = {
   ...
   */
  async readFile(worktreePath: string, filePath: string, encoding: BufferEncoding = 'utf-8'): Promise<string> {
