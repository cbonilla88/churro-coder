...
class LRUCache<T> {
  ...
  private evictIfNeeded(incomingSizeBytes: number): void {
    ...
    if (this.config.maxSizeBytes) {
      while (this.currentSizeBytes + incomingSizeBytes > this.config.maxSizeBytes && this.cache.size > 0) {
        this.evictLRU();
      }
    }
  }
...
export function estimateSizeBytes(value: unknown): number {
  ...
  if (typeof value === 'object') {
    return Object.entries(value).reduce((sum, [key, val]) => sum + key.length * 2 + estimateSizeBytes(val), 64);
  }
  return 0;
}
...
class GitCache {
  ...
  getFileContentIfHashMatches(worktreePath: string, filePath: string, contentHash: string): string | null {
    const key = `${worktreePath}:${filePath}`;
