...
export const anthropicAccountsRouter = router({
  ...
  list: publicProcedure.query(() => {
    ...
    try {
      const legacyCred = db.select().from(claudeCodeCredentials).where(eq(claudeCodeCredentials.id, 'default')).get();
  ...
  setActive: publicProcedure.input(z.object({ accountId: z.string() })).mutation(({ input }) => {
    ...
    const account = db.select().from(anthropicAccounts).where(eq(anthropicAccounts.id, input.accountId)).get();
    ...
    db.update(anthropicAccounts).set({ lastUsedAt: new Date() }).where(eq(anthropicAccounts.id, input.accountId)).run();
