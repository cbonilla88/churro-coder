...
export class DarwinPlatformProvider extends BasePlatformProvider {
  ...
  override async detectShell(): Promise<string> {
    ...
    try {
      const { stdout } = await this.execCommand('sh', ['-c', `dscl . -read /Users/$(whoami) UserShell 2>/dev/null`]);
  ...
  override async detectLocale(): Promise<string> {
    ...
    try {
      const { stdout } = await this.execCommand('sh', ['-c', 'locale 2>/dev/null | grep LANG= | cut -d= -f2']);
  ...
  async installCli(sourcePath: string): Promise<{ success: boolean; error?: string }> {
    ...
    try {
      ...
      if (existsSync(installPath)) {
        await execAsync(`osascript -e 'do shell script "rm -f ${installPath}" with administrator privileges'`);
      }
