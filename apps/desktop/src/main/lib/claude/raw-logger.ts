...
export async function logRawClaudeMessage(sessionId: string, msg: unknown): Promise<void> {
  ...
  try {
    ...
    const needsNewFile = sessionId !== currentSessionId || (currentLogFile && (await shouldRotateLog(currentLogFile)));
    ...
    if (needsNewFile) {
      ...
      if (sessionId !== currentSessionId) {
        ...
        cleanupOldLogs().catch((err) => console.error('[raw-logger] Background cleanup failed:', err));
      }
    }
